...
public enum UserMenuCommands implements Command{
    ...
    AddAddress("^\\s*add\\s+address\\s+-country\\s+(?<country>.*)\\s+-city\\s+(?<city>.*)\\s+-street\\s+(?<street>.*)\\s+-postal\\s+(?<postal>-?\\d+)\\s*$"),
    DeleteAddress("^\\s*delete\\s+address\\s+-id\\s+(?<id>-?\\d+)\\s*$"),
    ListAddress("^\\s*list\\s+my\\s+addresses\\s*$"),
    AddCard("^\\s*add\\s+a\\s+credit\\s+card\\s+-number\\s+(?<cardNumber>-?\\d+)\\s+-ed\\s+(?<expirationDate>\\S+)\\s+-cvv\\s+(?<cvv>-?\\d+)\\s+-initialValue\\s+(?<initialValue>-?\\d+\\.?\\d+|\\d+)\\s*$"),
    ChargeCard("^\\s*Charge\\s+credit\\s+card\\s+-a\\s+(?<amount>-?\\d+\\.?\\d+|\\d+)\\s+-id\\s+(?<id>-?\\d+)\\s*$"),
