...
public enum UserMenuCommands implements Command{
    ...
    EditName("^\\s*edit\\s+name\\s+-fn\\s+(?<firstName>\\S+)\\s+(?<lastName>\\S+)\\s+-p\\s+(?<password>\\S+)\\s*$"),
    EditEmail("^\\s*edit\\s+email\\s+-e\\s+(?<email>\\S+)\\s+-p\\s+(?<password>\\S+)\\s*$"),
    EditPass("^\\s*edit\\s+password\\s+-np\\s+(?<newPass>\\S+)\\s+-op\\s+(?<oldPass>\\S+)\\s*$"),
    ShowInfo("^\\s*show\\s+my\\s+infp\\s*$"),
    AddAddress("^\\s*add\\s+address\\s+-country\\s+(?<country>\\S+)\\s+-city\\s+(?<city>\\S+)\\s+-street\\s+(?<street>\\S+)\\s+-postal\\s+(?<postal>\\S+)\\s*$"),
    DeleteAddress("^\\s*delete\\s+address\\s+-id\\s+(?<id>-?\\d+)\\s*$"),
    ListAddress("^\\s*list\\s+my\\s+addresses\\s*$"),
    AddCard("^\\s*add\\s+a\\s+credit\\s+card\\s+-number\\s+(?<cardNumber>-?\\d+)\\s+-ed\\s+(?<expirationDate>\\S+)\\s+-cvv\\s+-initialValue\\s+(?<initialValue>\\d+)\\s+$"),
    ChargeCard("^\\s*Charge\\s+credit\\s+card\\s+-a\\s+(?<amount>-?\\d+)\\s+-id\\s+(?<id>-?\\d+)\\s*$"),
